...
async def complete_mission(mission: str, session_id: Optional[str] = Cookie(None), db: Session = Depends(get_db)):
    if session_id is None:
        raise HTTPException(status_code=401, detail="No session ID provided")
    ...
    if user_quest:
        user_quest.completed = True
        db.commit()
        return {"success": True, "message": f"{mission} mission completed"}
    else:
        return {"success": False, "message": "UserQuest entry not found"}